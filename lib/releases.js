...
module.exports.findReleases = async ({ app, context }) => {
  ...
  const draftRelease = process.env['tag_name'] ? releases.find(r => r.tag_name === process.env['tag_name']) : releases.find(r => r.draft)
  const baseRelease = process.env['tag_from'] 
    ? releases.find(r => r.tag_name === process.env['tag_from'])
  ...
  if (baseRelease) {
    log({ app, context, message: `Base release: ${baseRelease.tag_name}` })
  } else {
    log({ app, context, message: `No base release found` })
  }

  return { draftRelease, baseRelease }
}
